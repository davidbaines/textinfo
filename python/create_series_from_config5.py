...
def generate_configs(config_file_path, overwrite=False):
    ...
    print(f"Found {len(sources)} sources in the first corpus pair")
    ...
    while len(sources) > 1:  # Now we stop when we reach the last source
        ...
        folder_existed = new_folder.exists()
        config_existed = (new_folder / 'config.yml').exists()
        translate_config_existed = (new_folder / 'translate_config.yml').exists()
        ...
        # Write the new config if needed
        if not config_existed or overwrite:
        ...
        if has_translate_config:
            if not translate_config_existed or overwrite:
