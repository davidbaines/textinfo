...
def write_csv(outfile, row_data, column_headers, mode="w"):
    ...
    with open(outfile, mode, encoding="utf-8", newline="") as csvfile:
        ...
        writer.writerow({"Series": "Written by F:/GitHub/davidbaines/textinfo/python/summarize_scores_test.py"})
...
def get_fieldnames():
    ...
    return all_fieldnames, omit, column_headers
...
def main() -> None:
    ...
    all_fieldnames, omit, column_headers = get_fieldnames()
    included_fieldnames = {
        fieldname for fieldname in all_fieldnames if fieldname not in omit
    }
    column_headers = [
        column_header
        for fieldname, column_header in all_fieldnames.items()
        if fieldname not in omit
    ]
    ...
    for experiment_config in experiment_configs:
        ...
        if effective_configs:
            ...
            experiment, no_pairs = get_config_data(
                effective_configs[0], included_fieldnames
            )

        else:  # Use the config.yml file
            experiment, no_pairs = get_config_data(
                experiment_config, included_fieldnames
            )
                                ...
                                if row["book"] == "ALL":
                                    if row["scorer"] == "BLEU":
                                        try:
                                            ...
                                            print(
                                                f"Can't read the BLEU score on row {i} of file {score_file}"
                                            )
                                            exit()
                                    else:
                                        score = row["score"]
                                    ...
                                    if steps == best_steps:
                                        ...
                                        experiment[
                                            f"Best {row['scorer']} {row['book']}"
                                        ] = score

                                    elif steps == last_steps:
                                        experiment["Last steps"] = steps
                                        experiment[
                                            f"Last {row['scorer']} {row['book']}"
                                        ] = score
                    else:
                        continue
    ...
    pprint(tr_experiments[0])
    pprint(column_headers)
    
    complete_experiments = [
        experiment for experiment in experiments if experiment["complete"]
    ]
    incomplete_experiments = [
        experiment for experiment in experiments if not experiment["complete"]
    ]

    write_csv(output_file, tr_experiments, column_headers=column_headers)
    print(
        f"Report includes all experiments. {len(complete_experiments)} experiments have scores and {len(incomplete_experiments)} do not."
    )
